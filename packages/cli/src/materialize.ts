...
export interface MaterializeOptions {
  /**
   * Directory whose node_modules resolves @ogs-tech/press-web. Defaults to the project
...
/**
 * Regenerates `<project>/.press/web/` from the @ogs-tech/press-web host template (spec
 * §4.1). The host lives INSIDE the project tree so Node resolution reaches the
 * root node_modules (@ogs-tech/press-web, next, react) and the adopter's blocks/custom/.
