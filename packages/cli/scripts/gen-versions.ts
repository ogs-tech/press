 ...
 *
 *   pnpm --filter @press/cli gen:versions          # rewrite the committed file
 *   pnpm --filter @press/cli gen:versions --check   # exit 1 if the file is stale (CI)
 */
...
if (process.argv.includes('--check')) {
  ...
  if (current !== rendered) {
    console.error('versions.generated.ts is stale — run: pnpm --filter @press/cli gen:versions');
    process.exit(1);
  }
