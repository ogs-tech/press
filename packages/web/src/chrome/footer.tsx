 ...
 * the outer element is now a `<Container>` (owns gutter — the shell's
 * `<footer>` keeps only vertical padding + the border stroke after the Task 14
 * shell rewrite). `maxWidth="full"` matches the Navbar so both chrome surfaces
 * share edge-to-edge geometry: chrome is full-bleed, content Containers are
 * the constrained ones. Empty `text` falls back to "brand · currentYear" —
