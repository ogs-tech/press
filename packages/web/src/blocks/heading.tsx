...
/**
 * Atom `preset-atom.heading` — a standalone section heading (h1–h6). The
 ...
 */
export function Heading({ text, level }: PresetAtomHeading) {
  const Tag = `h${level ?? '2'}` as 'h1' | 'h2' | 'h3' | 'h4' | 'h5' | 'h6';
  return <Tag data-block="preset-atom.heading">{text}</Tag>;
}
