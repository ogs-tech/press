...
- **Outcome:** **PASS on Path A.** Strapi 5 can ship as a versioned, updatable dependency (`@press/cms`) — content-types **and** Dynamic-Zone blocks — consumed by a thin owned host, and `pnpm update @press/cms` upgrades the engine without touching the adopter's Project zone beyond the dependency version line.
- **Strapi pin:** `5.48.0` (spec §4, §11).
- **Plugin load mechanism:** Strapi **auto-discovery** — the host enables the engine with `{ "press-cms": { enabled: true } }` (no explicit `resolve`); Strapi finds `@press/cms` from the host's `dependencies` because the package ships `strapi.kind: "plugin"`. This replaced the plan's `resolve: "@press/cms"`: an explicit `resolve` forces `dirname(require.resolve("@press/cms"))`, which needs a root `./`/`main` export, and adding one makes `strapi-plugin build` clean the package root and delete the engine source. The engine keeps the clean sdk-plugin exports (only `./package.json` + `./strapi-server`).
- **Registry isolation:** `@press/cms` published to a local Verdaccio (`scripts/registry.sh`). `.npmrc` sets `link-workspace-packages=false` so the host always consumes the published **tarball** (`apps/cms/node_modules/@press/cms` contains only `dist/`), never the workspace source — otherwise the update proof would test a symlink instead of real semver (spec §7).
- **T1 / pivot decision:** **Path A.** Evidence: A2 — the engine's plugin `register` lifecycle injects `press.hero` into `strapi.get('components')` (mirroring the shape from `@strapi/core/dist/loaders/components.js`) before `bootstrap()` builds DB models; the admin Content-Type Builder API lists `press.hero` (heading/subheading/ctaLabel) and a `Page` entry with a `press.hero` block saved via the content-manager API (HTTP 201). A1 (declarative `components/hero.json` in the plugin) is not auto-registered — confirmed there is no declarative plugin-component API; Strapi only scans the host app's `src/components`.
- **T3 / adopter custom block (most likely leak site — held):** the adopter drops `apps/cms/src/components/custom/callout.json` (Project zone). The engine admits ALL `custom.*` components into its `page` dynamic zone at `register` time via a generic prefix convention — it never names `callout`. Verified: the dynamic zone offers both `Hero` (engine) and `Callout` (adopter), and a `Page` with both blocks saves (HTTP 201). `scripts/assert-no-engine-in-host.mjs` confirms no engine code lives in the host `src/`.
- **Acceptance (spec §8) across v0.1.0 → v0.2.0** (`scripts/contract-check.mjs 0.1.0 0.2.0` → `CONTRACT HELD`, exit 0):
  1. **Allowed delta only** — the update changed exactly `apps/cms/package.json` (the `@press/cms` version line) + `pnpm-lock.yaml`; nothing in the Project zone.
  2. **Builds** — `pnpm --filter cms build` exits 0 after the update.
  3. **Boots** — host reaches `/_health` 204; admin still shows engine `Page`, reference `Hero`, and adopter `Callout`; a `Page` with both blocks saves (HTTP 201) on 0.2.0. v0.2.0 was an INTERNAL-only restructure (register logic split into `lib/inject-components.ts`); the public `press.hero` id/attributes and the `custom.*` interface are unchanged.
  4. **Deploy** — documentary check: `strapi ts:generate-types` succeeds; `config/server.ts` and `config/database.ts` remain env-driven (no real deploy executed).
- **Contract test:** `scripts/contract-check.mjs` retained as the seed of the CI guard (PRD §8). Hardening surfaced three real bugs in it, now fixed: (a) `.trim()` on the porcelain snapshot stripped the first line's leading status space, mis-slicing the first changed path; (b) `sh()` crashed on `execSync`'s `null` return under `stdio: "inherit"` (the build step); (c) Strapi's verbose boot output flooded the `execSync` pipe — redirected to a log file.
- **Date:** 2026-06-11.
