...
- **Responsiveness never appears in the JSON.** `ratio`/`gap`/`width` are editorial
  intents; the web renderer maps them to `Responsive<T>` values (the `RATIO_SPANS` /
...
- `RowNode` at top level → `<Container maxWidth={width}><Grid gap alignItems>`;
  nested inside a Column → bare `<Grid>` filling the cell.
